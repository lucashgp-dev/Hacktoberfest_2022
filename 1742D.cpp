...
void solve() {
    ...
    vector<pair<ll,ll>> fl(1001,{-1,-1});
    forn(i,n) {
        if(fl[v[i]].ff==-1) fl[v[i]].ff=i+1;
        fl[v[i]].ss=max(fl[v[i]].ss,i+1);
    }
    ll ans=-1;
    forn1(i,1000) {
        forn1(j,1000) {
            if(gcd(i,j)!=1) continue;
            if(fl[i].ff==-1) continue;
            if(fl[j].ff==-1) continue;
            ans=max(ans,fl[j].ss+fl[i].ss);
        }
    }
    cout<<ans<<endl;
}
